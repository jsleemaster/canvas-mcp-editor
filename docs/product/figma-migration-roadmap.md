...
- Wrap, fallback gap, row/column gap split, padding, child item margins, static/absolute layout item positioning, wrapped-line distribution, and container fit width/height sizing.
...
- Add inspector controls for layout mode, flow, wrap, alignment, distribution, fallback gap, row/column gap, padding, child margins, child layout-item positioning, container fit sizing, and constraints.
- Add Playwright coverage for automatic sibling repositioning and parent resize behavior.

Non-goals for the first slice:

- Grid auto layout.
- Child fill sizing, min/max rules, baseline alignment, and advanced intrinsic sizing rules.
...
1. Keep the landed Lane 1 layout foundation, Flex alignment, child-margin flow, static/absolute layout item positioning, wrap, row/column gap split, container fit sizing, and PR #20 navigation fixes green.
