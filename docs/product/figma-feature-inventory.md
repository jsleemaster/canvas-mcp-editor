...
| Fills and imagery | Solid fills, gradients, image fills, patterns, color picker. | Solid fill plus local image asset storage, image nodes inserted by clipboard paste or file drag/drop, context-menu `이미지 바꾸기`, and context-menu `원본 크기로 맞춤` for uploaded images. | Adopt. Add gradients, image crop controls, and image-fill controls next; patterns later. |
