...
| Layer model | Pages, frames, shapes, text, images, groups, sections, parent/child hierarchy, selection. | Pages, frames, transparent groups, rectangles, text, images in type model, nested children, image insertion by paste/drop, layer ordering, node lock/visibility state, context-menu group/ungroup, and context-menu frame selection for sibling layers. No sections UI. | Adopt. Add richer layer nesting controls, section-like organization, and more object context-menu groups. |
