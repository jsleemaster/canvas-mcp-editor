...
| Layer model | Pages, frames, shapes, text, images, groups, sections, parent/child hierarchy, selection. | Pages, frames, rectangles, text, images in type model, nested children, image insertion by paste/drop, layer ordering, and node lock/visibility state. No groups/sections UI. | Adopt. Add group/ungroup, richer layer nesting controls, and more object context-menu groups. |
